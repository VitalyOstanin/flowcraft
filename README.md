...
| Термин | Описание |
|--------|----------|
| **Workflow** | Последовательность этапов для выполнения определенной задачи (разработка проекта, исправление багов, анализ инцидентов). Определяется в YAML файле с указанием агентов и этапов |
| **Этапы (Stages)** | Отдельные шаги в рамках workflow, каждый из которых выполняется определенным агентом. Могут быть обязательными или пропускаемыми |
| **Агенты** | AI помощники с уникальными именами (developer-basic, architect-advanced), системными промптами, привязанными LLM моделями и наборами возможностей. Хранятся в отдельных YAML файлах в `~/.flowcraft/agents/` |
| **Модели** | LLM провайдеры для генерации ответов (qwen3-coder-plus, kiro-cli). Делятся на дешевые для обычных задач и дорогие для сложных этапов |
